...
# (c) 2015, Nick Bales <nick.bales@rackspace.com>
#
# This file is an extension of the bigip_monitor_tcp module.
#

DOCUMENTATION = '''
---
module: bigip_monitor_database
short_description: "Manages F5 BIG-IP LTM database monitors"
description:
    - "Manages F5 BIG-IP LTM database monitors via iControl SOAP API"
version_added: "1.4"
author: "Nick Bales"
notes:
    - "Requires BIG-IP software version >= 11.5.1 HF6"
    - "May work with previous versions of 11.x, but will likely encounter buggy results (see https://support.f5.com/kb/en-us/solutions/public/14000/800/sol14862.html)"
...
options:
    ...
    db_connection_count:
        description: 
            - number of database monitor probes before the system terminates connection
        required: false
        default: 1
    ip:
        description: 
...
- name: BIGIP F5 | Create PostgreSQL Monitor
  local_action:
    module:              bigip_monitor_database
    state:               present
    server:              "{{ f5server }}"
    ...
    db_connection_count: "{{ item.db_connection_count }}"
  with_items: f5monitors-database
- name: BIGIP F5 | Remove database Monitor
  local_action:
    module:             bigip_monitor_database
    state:              absent
    server:             "{{ f5server }}"
...
TEMPLATE_TYPE = DEFAULT_TEMPLATE_TYPE = 'mysql'
TEMPLATE_TYPE_CHOICES = ['mysql', 'postgresql', 'mssql']
try:
    import bigsuds
except ImportError:
    bigsuds_found = False
...
def main():
    ...
    argument_spec.update(dict(
            ...
            db_connection_count = dict(required=False, type='int'),
    ...
    db_username = modules.params['db_username']
    db_password = modules.params['db_password']
    db_name = modules.params['db_name']
    db_connection_count = modules.params['db_connection_count']
    ip = module.params['ip']
